...
absl::Status ListInstances(RequestContext* ctx,
  ...
  if (!request->page_token().empty()) {
    ...
    GOOGLESQL_RETURN_IF_ERROR(ParseInstanceUri(request->page_token(), &project_id, &instance_id));
  }

  GOOGLESQL_ASSIGN_OR_RETURN(std::vector<std::shared_ptr<Instance>> instances,
...
absl::Status DeleteInstance(RequestContext* ctx,
  ...
  GOOGLESQL_ASSIGN_OR_RETURN(std::vector<std::shared_ptr<Database>> databases,
  ctx->env()->database_manager()->ListDatabases(request->name()));
  for (const auto& database : databases) {
    GOOGLESQL_ASSIGN_OR_RETURN(std::vector<std::shared_ptr<Session>> sessions,
